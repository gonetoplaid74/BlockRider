...
class GameViewController: UIViewController, SCNSceneRendererDelegate, SCNPhysicsContactDelegate, GKGameCenterControllerDelegate {
    ...
    func createBox(){
        ...
        if speedName == "Ludicrous" && counter <= 10 && counter >= 1 && counter % 2 == 0{
            addScore()
        } else if speedName == "Hyper-Active" && counter <= 10 && counter >= 1 && counter % 3 == 0{
            addScore()
        } else if speedName == "Slow" && counter <= 10 && counter >= 1 && counter % 5 == 0 {
            addScore()
        }
        if counter == 15 {
            if speed <= maxSpeed {
    ...
    func bonusAction() {
        ...
        if currentBonus == "Green" && bonusSpeed >= 0.75 {
            var percentage = Float(speed)
            percentage = percentage * 0.1
            var multiplier = Int()
            multiplier = Int(percentage)
