...
class GameViewController: UIViewController, SCNSceneRendererDelegate, SCNPhysicsContactDelegate, GKGameCenterControllerDelegate {
    ...
    func physicsWorld(_ world: SCNPhysicsWorld, didBegin contact: SCNPhysicsContact) {
        ...
        if nodeA.physicsBody?.categoryBitMask == bodyNames.Bonus && nodeB.physicsBody?.categoryBitMask == bodyNames.Person{
    ...
    func createBonus(_ box : SCNNode) {
        ...
        if randomNuber == 1 {
            let bonusScene = SCNScene(named: "pink.dae")
            let bonus = bonusScene?.rootNode.childNode(withName: "pink", recursively: true)
            bonus?.position = SCNVector3Make(box.position.x, box.position.y, box.position.z)
            bonus?.scale = SCNVector3Make(0.2, 0.2, 0.2)
